...
// Simulación de iconos de redes sociales
const socialLinks = [
  {
    name: "Facebook",
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-2 gap-4">
      {socialLinks.map((social, index) => (
        ...
        >
          <div className="relative bg-linear-to-br from-[#262626] to-[#171717] rounded-xl p-4 border border-[#404040] hover:border-[#737373] transition-all duration-300 hover:scale-105 hover:shadow-2xl overflow-hidden">
            ...
            <div className="relative flex items-center justify-center">
              <svg
                viewBox="0 0 24 24"
                className="w-8 h-8 fill-[#A3A3A3] group-hover:fill-white transition-colors duration-300"
              >
                <path d={social.icon} />
              </svg>
            </div>
            ...
            {/* Nombre de la red */}
            <p className="text-xs text-center mt-2 text-[#A3A3A3] group-hover:text-white transition-colors duration-300 font-medium">
              {social.name}
            </p>

            {/* Icono de link externo */}
            <ExternalLink className="absolute top-2 right-2 w-3 h-3 text-[#525252] opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          </div>
        </a>
      ))}
    </div>
